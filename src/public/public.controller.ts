 ...
 *   GET /          — form (or "closed" if survey_open=false)
...
export class PublicController {
  ...
  async index(
    ...
    if (!(await this.registrations.isSurveyOpen())) {
      res.status(200);
      // Closed page — shorter cache (10s) so it "opens" quickly when
      // admin reopens the survey
      res.header(
        'Cache-Control',
